  ...
  return (
    <div className={styles.guideContent}>
      ...
      <p>
        ...
        <strong>more light</strong>, the resistance is lower. When there is less
        light, the resistance is higher. The specific amount of resistance
        depends on the specific component.{' '}
        <a href="https://www.instructables.com/Photoresistors/" target="_blank">
          Instructables
        </a>{' '}
      ...
      />
      <p>
        Photoresistors are non-polarized (no notion of + or -), meaning you can
        use it in either direction.
      </p>
      <p>
        One way to use the photoresistor is to create a voltage divider where
        one resistor is the photoresistor, and the other is a regular resistor,
        as seen in the diagram below. To value of A0 will change depending on
      ...
      />
      <p>
        The value (voltage) of A0 (also known as Analog Pin 0) is a fraction of
        5v. The voltage divider makes it such that the voltage at that point is:
      </p>
      ...
      />
      <p>
        In this example, when the photoresistor is exposed to bright light, the
        resistance is 10k&#8486;, meaning the top and bottom resistances are
        both 10k&#8486;, meaning that A0 is split right down the middle (half of
        5V) which is <strong>2.5V</strong>.
      </p>
      <p>
        When the photoresistor is in darkness, the resistance is very large
        (100k&#8486;); the ratio is roughly 10:1 between the top resistance and
        the bottom resistance. Intuitively, this pushes A0 in value much closer
        to ground. If we do the math, it gives a reading of &lt; 0.5v.
      </p>
      <h2>Let's wire it up!</h2>
      <p>
        To wire this up, we'll use the 5V and the Gnd pins on the Arduino. One
        new pin we'll be using is the A0, the Analog pin. This pin allows us to
        take in values that are between 0 and 5v. The full documentation for{' '}
        <code>analogRead()</code> can be found on the{' '}
        <a
      ...
      <p>
        Instead of doing <code>Serial.println(value)</code> statement, print the
        value of the raw <code>sensorValue</code> and play around with what you
        get. Shine a bright light at it, what is the number? Then cover it
        completely, and get that number. These are your light and dark values!
      </p>
      <p>
